...
from hummingbot.connector.exchange.switcheo.switcheo_order_book_tracker import CryptoComOrderBookTracker
from hummingbot.connector.exchange.switcheo.switcheo_user_stream_tracker import CryptoComUserStreamTracker
from hummingbot.connector.exchange.switcheo.switcheo_auth import CryptoComAuth
from hummingbot.connector.exchange.switcheo.switcheo_in_flight_order import CryptoComInFlightOrder
from hummingbot.connector.exchange.switcheo import switcheo_utils
from hummingbot.connector.exchange.switcheo import switcheo_constants as Constants
from hummingbot.core.data_type.common import OpenOrder
ctce_logger = None
s_decimal_NaN = Decimal("nan")


class SwitcheoExchange(ExchangeBase):
    """
    ...
    def __init__(self,
        ...
        self._switcheo_auth = CryptoComAuth(crypto_com_api_key, crypto_com_secret_key)
        self._order_book_tracker = CryptoComOrderBookTracker(trading_pairs=trading_pairs)
        self._user_stream_tracker = CryptoComUserStreamTracker(self._switcheo_auth, trading_pairs)
        ...
        while True:
            try:
                ...
                self.logger().network(f"Unexpected error while fetching trading rules. Error: {str(e)}",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Switcheo. "
                                                      "Check network connection.")
                await asyncio.sleep(0.5)

    async def _update_trading_rules(self):
        instruments_info = await self._api_request("get", path_url="public/get_markets")
    ...
    def _format_trading_rules(self, instruments_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for rule in instruments_info:
            try:
                trading_pair = switcheo_utils.convert_from_exchange_trading_pair(rule["display_name"])
    ...
    async def _api_request(self,
        ...
        if is_auth_required:
            request_id = switcheo_utils.RequestId.generate_request_id()
            data = {"params": params}
            params = self._switcheo_auth.generate_auth_dict(path_url, request_id,
                                                              switcheo_utils.get_ms_timestamp(), data)
            headers = self._switcheo_auth.get_headers()
        else:
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        """
        order_id: str = switcheo_utils.get_new_client_order_id(True, trading_pair)
    ...
    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        :returns A new internal order id
        """
        order_id: str = switcheo_utils.get_new_client_order_id(False, trading_pair)
    ...
    async def _create_order(self,
        ...
        create_order_msg = types.CreateOrderMessage(market=switcheo_utils.convert_to_exchange_trading_pair(trading_pair), side=trade_type.name, quantity=amount, price=price, type="limit")
        ac = AuthenticatedClient(self.newWallet, None, None, "mainnet")
        
        api_params = {"instrument_name": switcheo_utils.convert_to_exchange_trading_pair(trading_pair),
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} {order_type.name} order to Switcheo for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> str:
        ...
        try:
            ...
            #     {"instrument_name": switcheo_utils.convert_to_exchange_trading_pair(trading_pair),
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Switcheo. "
    ...
    def _process_order_message(self, order_msg: Dict[str, Any]):
        ...
        if tracked_order.is_cancelled:
            ...
            self.logger().info(f"The market order {client_order_id} has failed according to order status API. "
                               f"Reason: {switcheo_utils.get_api_reason(order_msg['reason'])}")
            self.trigger_event(MarketEvent.OrderFailure,
                               MarketOrderFailureEvent(
                                   self.current_timestamp,
                                   client_order_id,
    ...
    async def cancel_all(self, timeout_seconds: float):
        ...
        try:
            ...
            for trading_pair in self._trading_pairs:
                ...
                #     True
                # )
                message=types.CancelAllMessage(market=switcheo_utils.convert_to_exchange_trading_pair(trading_pair))
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel all orders on Switcheo. Check API key and network connection."
            )
    ...
    async def get_open_orders(self) -> List[OpenOrder]:
        ...
        for order in result.values():
            if switcheo_utils.HBOT_BROKER_ID not in order["client_oid"]:
                continue
            ...
            ret_val.append(
                OpenOrder(
                    ...
                    trading_pair=switcheo_utils.convert_from_exchange_trading_pair(order["market"]),
