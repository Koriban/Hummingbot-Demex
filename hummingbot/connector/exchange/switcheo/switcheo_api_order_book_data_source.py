...
from .switcheo_active_order_tracker import CryptoComActiveOrderTracker
from .switcheo_order_book import CryptoComOrderBook
from .switcheo_websocket import CryptoComWebsocket
        ...
        while True:
            try:
                ...
                await ws.subscribe(list(map(
                    lambda pair: f"trade.{switcheo_utils.convert_to_exchange_trading_pair(pair)}",
                    self._trading_pairs
                )))
                ...
                async for response in ws.on_message():
                    ...
                    for trade in response["result"]["data"]:
                        ...
                        trade_msg: OrderBookMessage = CryptoComOrderBook.trade_message_from_exchange(
                            trade,
                            trade_timestamp,
                            metadata={"trading_pair": switcheo_utils.convert_from_exchange_trading_pair(trade["i"])}
                        )
            ...
            try:
                for trading_pair in self._trading_pairs:
                    try:
                        ...
                        snapshot_timestamp: int = ms_timestamp_to_s(1624534710839) # set the timestamp static for now
                        snapshot_msg: OrderBookMessage = CryptoComOrderBook.snapshot_message_from_exchange(
                            snapshot,
                            snapshot_timestamp,
                            metadata={"trading_pair": trading_pair}
